...
def plot_task_type(task_name, experiment_name):
    ...
    # Create one confusion matric for each testing session
    ...
    for ax, session in zip(axes, SESSIONS):
        ...
        # Add the authentication counts to each cell
        max_val = matrix.max()
        for i in range(2):
            for j in range(2):
                val = matrix[i, j]
    ...
    # Save the finished figure to the dissertation figures folder
...
# Generate confusion matric figures for every experiment type
